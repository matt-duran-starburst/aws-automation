...
from botocore.exceptions import NoCredentialsError, ClientError, NoCredentialsError, EndpointConnectionError
...
"""
S3-bucket command, which is a subcommand of 'create' 
"""
@create.command("s3-bucket")
@click.option('--name', required=True, help="The name of the S3 bucket to manage.")
@click.option('--region', help="The AWS region for the S3 bucket operation. Overrides config default.", default=None)
@click.option('--owner-email', help="The owner's email (e.g., user@example.com) for tags. Defaults to config user email.", default=None)
@click.option('--preset', help="The deployment preset value for the 'info' tag. Defaults to config environment.", default=None)
@click.option('--expires-days', type=int, default=90, help="Number of days until bucket expiration tag. Defaults to 90.")
def s3_bucket_command(name, region, owner_email, preset, expires_days):
    """
    Locate or create a new S3 bucket, applying dynamic tags.
    """
    # Instantiate PlatformConfig directly within the command function
    config_instance = PlatformConfig()
    _config = config_instance.config 

    # Determine region: CLI option first, then config's aws_region, then config's default_region
    actual_region = region or _config.get("aws_region") or _config.get("default_region")
    if not actual_region:
        click.echo(click.style("Error: AWS region not provided via --region or found in config (aws_region/default_region).", fg='red'), err=True)
        return 1
    # Sanitize actual_region immediately after determination
    actual_region = str(actual_region).strip().lower()


    # Determine owner_email: CLI option first, then config's user_profile.email, then config's user_profile.name
    actual_owner_email = owner_email or _config.get("user_profile", {}).get("email") or _config.get("user_profile", {}).get("name")
    if not actual_owner_email:
        click.echo(click.style("Error: Owner email not provided via --owner-email or found in config (user_profile.email/name).", fg='red'), err=True)
        return 1

    # Determine preset: Now always derived from config. No CLI option to override.
    actual_preset = preset or _config.get("environment") or _config.get("default_tags", {}).get("environment") or "development"
    if not actual_preset: 
        click.echo(click.style("Error: Preset could not be determined from config. Defaulting to 'development'.", fg='yellow'), err=True)
        actual_preset = "development"


    click.echo(f"\n--- Orchestrating S3 bucket management for '{name}' in region '{actual_region}' ---")

    # Construct the string for parse_expiration from the integer 'expires_days'
    expires_in_str = f"{expires_days}d"
    try:
        expires_at_value = parse_expiration(expires_in_str)
    except click.BadParameter as e:
        click.echo(click.style(f"Error: Invalid --expires-days value (internal conversion failed): {e}", fg='red'), err=True)
        return 1


    # --- Step 1: Attempt to locate the bucket ---
    located_bucket = locate_s3_bucket(name, actual_region)

    if located_bucket:
        click.echo(click.style(f"Bucket '{located_bucket}' already exists. No creation needed.", fg='green'), err=True)
        return 0
    else:
        click.echo(f"Bucket '{name}' not found. Attempting to create it...")
        # Step 2: Create the bucket if it wasn't located
        # MODIFIED: Pass actual_region as 'region' argument to create_s3_bucket
        if create_s3_bucket(
            name,
            actual_region, # Pass the resolved region explicitly
            config_instance, 
            actual_owner_email,
            actual_preset, 
            expires_at_value
        ):
            click.echo(click.style(f"Bucket '{name}' successfully created and tagged.", fg='green'), err=True)
        else:
            click.echo(click.style(f"Failed to create bucket '{name}'. See errors above.", fg='red'), err=True)
            return 1 
...
def locate_s3_bucket(name: str, region: str) -> str | None:
    """
    Checks if an S3 bucket exists. Validates AWS credentials before checking.
    """
    # Sanitize region immediately upon entry
    region = str(region).strip().lower() 
    if not region: # Handle empty string after strip
        click.echo(click.style("Error: Region is empty after sanitization in locate_s3_bucket.", fg='red'), err=True)
        return None

    try:
        # Call validate_aws_credentials without 'region' as per constraint
        identity = validate_aws_credentials() 
    except (NoCredentialsError, ClientError, EndpointConnectionError, Exception):
        click.echo(click.style(f"Pre-check failed for bucket '{name}' due to credential or connection issues. Cannot locate.", fg='red'), err=True)
        return None
    ...
    try:
        ...
        click.echo(f"Bucket '{name}' found.")
        return name
    except ClientError as e:
        error_code = e.response.get('Error', {}).get('Code')
        if error_code == '404':
            click.echo(f"Bucket '{name}' does not exist.")
            return None
        else:
            click.echo(click.style(f"Error checking bucket '{name}': {e}", fg='red'), err=True)
            return None
    except Exception as e:
        click.echo(click.style(f"An unexpected error occurred while checking bucket '{name}': {e}", fg='red'), err=True)
        return None
...
def create_s3_bucket(
    ...
    region = str(region).strip().lower()
    if not region: # Handle empty string after strip
        click.echo(click.style("Error: Region is empty after sanitization in create_s3_bucket.", fg='red'), err=True)
        return False

    _config = config_instance.config 
    
    click.echo(f"Debug: Attempting to create bucket in region '{region}' (type: {type(region)}).")
    ...
    try:
        ...
        if region == 'us-east-1':
            try:
                ...
                if error_code in ['IllegalLocationConstraintException', 'MalformedXML']:
                    ...
                    try:
                        ...
                        click.echo(click.style(f"Error: Failed to create or tag bucket '{name}' even on us-east-1 retry: {retry_ce}", fg='red'), err=True)
                        return False
                    except Exception as retry_unhandled_ce:
            ...
            click.echo(f"Debug: Called s3.create_bucket(Bucket='{name}', CreateBucketConfiguration={{'LocationConstraint': '{region}'}}) for non-us-east-1.")
        ...
        cluster_tags["Environment"] = _config.get("environment", _config.get("default_tags", {}).get("environment", "unknown_env"))
        
        # --- FIX FOR ENVIRONMENT DUPLICATION ---
        if 'environment' in cluster_tags:
            click.echo("Debug: Removing duplicate 'environment' (lowercase) tag.")
            del cluster_tags['environment']
        # --- END FIX ---

        click.echo(f"Generated tags for bucket: {json.dumps(cluster_tags, indent=2)}")

        s3_tags_list = [{'Key': k, 'Value': v} for k, v in cluster_tags.items()]

        # --- Separate put_bucket_tagging ---
        ...
        click.echo(click.style(f"Error: Failed to create or tag bucket '{name}': {ce}", fg='red'), err=True)
        return False
    except Exception as unhandled_ce:
        click.echo(click.style(f"Error: An unexpected error occurred during bucket creation for '{name}': {unhandled_ce}", fg='red'), err=True)
        return False
